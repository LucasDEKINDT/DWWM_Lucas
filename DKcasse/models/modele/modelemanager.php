...
class Modelemanager extends Model{
    ...
    public function getModeleById($idmodele){
        $req = $this->getBdd()->prepare("SELECT * FROM modeles WHERE IdModele = :IdModele");
        $req->bindValue(":IdModele", $idmodele, PDO::PARAM_INT);
        $req->execute();
        $modele = $req->fetch(PDO::FETCH_ASSOC);
        $req->closeCursor();
        ...
        return new Modele($modele['IdModele'], $modele['NomModele'], $modele['Annee']);
    }
